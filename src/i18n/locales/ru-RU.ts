...
/example\.(net|org)/`,
  ...
  integrations: {
    ...
    api: {
      title: 'Интеграция API Key',
      subtitle: 'Используйте API Key арендатора для серверной интеграции и выберите, как передавать идентификатор конечного пользователя.',
      ...
      principalModeDesc:
        'Определяет, как запросы с X-API-Key сопоставляются с Principal. Сейчас Principal изолирует только MCP OAuth и связанные потоки и не сужает область API ключа уровня admin арендатора.',
      principalScope:
        'Изоляция Principal распространяется только на токены MCP OAuth и авторизацию MCP в чате. API key по-прежнему имеет доступ admin арендатора; базы знаний, сессии и другие ресурсы не изолируются по внешнему пользователю.',
      ...
      requireDirectHeaderDesc: 'Если включено, запросы без заголовка ID пользователя отклоняются; иначе используется Principal уровня арендатора.',
